...
Filenames={...
    '0007B02R01Cam01P0WT-CantonS.avi';...
    '0007B02R01Cam02P0WT-CantonS.avi';...
    '0007B02R01Cam03P0WT-CantonS.avi';...
    '0007B02R01Cam04P0WT-CantonS.avi';...
    };
...
for lfilename=1:length(Filenames)
    ...
    FlytracksNewL=nan(length(frames2analyse),2);
    FlytracksNewC=nan(length(frames2analyse),2);
    FlytracksNewR=nan(length(frames2analyse),2);
    % and this, if we need
    xrct=[];
    yrct=[];
    xrht=[];
    ...
    for lframe=frames2analyse
        ...
        for arenaside=1:3
            ...
            % % % %
            % % % % Note:
            % % % % I need to include the mesh to prevent dark sides outside the arena
            % % % % now do it manually (maybe it is faster)--> Vero solves this using
            % % % % a mask.
            % % % idxCut=25;
            % % % i1(1:size(i1,1),1:idxCut)=255;
            % % % i1(1:size(i1,1),size(i1,2)-idxCut:size(i1,2))=255;
            % % % i1(1:idxCut,1:size(i1,2))=255;
            % % % i1(size(i1,1)-idxCut:size(i1,1),1:size(i1,2))=255;
            % % %
            % % %
            % % % i10=i1;
            % % % % thresholding, which is illumination and arean dependent
            % % % im2bwThreshold=0.6; % NOTE: HEURISTIC VALUE (I think Vero might have used something like 0.74)
            % % % i2=im2bw(i10,im2bwThreshold);
            % % % %
            % % % % % other stuff we could do to quickly smooth (and avoid spurs later)
            % % % %se2 = strel([1; 1; 1]);
            % % % %se2= strel('ball',1,2);
            % % % %i=imdilate(i2,se2);
            % % % %%i=i2;
            % % % % set depending on white object on black background or the opposite
            % % % i=1-i2;
...
%                 % else of the Arees0 condition
    ...
    atoc=toc
    save(['C:\Users\Vero\Documents\Tracking Data\Experiments\AlexTracking\TotaltrakingCent-' filename(1:end-4) '-Left.mat'],'FlytracksNewL','atoc','param','filename')
    save(['C:\Users\Vero\Documents\Tracking Data\Experiments\AlexTracking\TotaltrakingCent-' filename(1:end-4) '-Centre.mat'],'FlytracksNewC','atoc','param','filename')
    save(['C:\Users\Vero\Documents\Tracking Data\Experiments\AlexTracking\TotaltrakingCent-' filename(1:end-4) '-Right.mat'],'FlytracksNewR','atoc','param','filename')
