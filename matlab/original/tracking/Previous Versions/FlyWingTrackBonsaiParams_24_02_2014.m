...
param.arenaThreshold=170;

Filenames={...
    '0003A01R02Cam03P0WT-CantonS.avi';...
    '0003A01R02Cam02P0WT-CantonS.avi';...
};
...
for lfilename=1:length(Filenames)
    ...
    WingtracksNewL=nan(length(frames2analyse),3);
    WingtracksNewC=nan(length(frames2analyse),3);
    WingtracksNewR=nan(length(frames2analyse),3);
...
%             regprops=regionprops(L,'Centroid','Area','Orientation','MajorAxisLength','MinorAxisLength');%,'Perimeter'
            ...
            regprops=regionprops(L,i1,'WeightedCentroid','Area');% For Wing centroid
            ...
            if Arees0(bigobject1)>Athr % this could be skipped if we are confident
                ...
                wingcentr=regprops(bigobject).WeightedCentroid;
...
%                 % else of the Arees0 condition
            ...
            if arenaside==1
                WingtracksNewL(frameCount,:)=[xwc ywc A];% xrh yrh xrt yrt];
                
            elseif arenaside==2
                WingtracksNewC(frameCount,:)=[xwc ywc A];% xrh yrh xrt yrt];
                
            else
                WingtracksNewR(frameCount,:)=[xwc ywc A];% xrh yrh xrt yrt];
...
%                 save(['tracking_10000_' filename(1:end-4) '-Centre.mat'],'WingtracksNewC','param','filename')
...
%             end
    ...
    atoc=toc
    save(['C:\Users\Vero\Bonsai tracking\WingTrackingBonsaiParams-' filename(1:end-4) '-Left.mat'],'WingtracksNewL','atoc','param','filename')
    save(['C:\Users\Vero\Bonsai tracking\WingTrackingBonsaiParams-' filename(1:end-4) '-Centre.mat'],'WingtracksNewC','atoc','param','filename')
    save(['C:\Users\Vero\Bonsai tracking\WingTrackingBonsaiParams-' filename(1:end-4) '-Right.mat'],'WingtracksNewR','atoc','param','filename')
