function h=plot_tracks_single2(FlyDB,X_Y,lfly,Spots,params,px,Color,range,FontSz,plotarena,LW)
...
%     clf
Geometry = FlyDB(lfly).SubstrateType';
...
if plotarena==1
    ...
    for nspot=YSpots
        [c,xc,yc]=circle_([FlyDB(lfly).PatchPositions(nspot,1)*params.px2mm,...
            FlyDB(lfly).PatchPositions(nspot,2)*params.px2mm],...
            1.6,100,'-b');%1.6%2.5
...
%         set(c,'Color',CmapSubs2(Geometry(nspot),:),'LineWidth',2)
        c=circle_([FlyDB(lfly).PatchPositions(nspot,1)*params.px2mm,...
            FlyDB(lfly).PatchPositions(nspot,2)*params.px2mm],...
           2.5,100,':');% 2.5 %5
...
font_style({[params.condition_labels{params.condition_index(lfly)},...
