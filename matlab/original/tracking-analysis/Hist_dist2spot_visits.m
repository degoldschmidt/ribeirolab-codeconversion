...
MeandistVFlies=nan(max(fliescond(lcond)),length(unique(params.ConditionIndex)));
...
%             Alldist_Cond{params.ConditionIndex(lfly)}=[Alldist_Cond{params.ConditionIndex(lfly)};Dist2fSpot];
    ...
    MeandistVFlies(Flycondcounter(params.ConditionIndex(lfly)),params.ConditionIndex(lfly))=mean(Alldists);
...
plot_boxplot_Fig2(MeandistVFlies(:,Conditions),labels2,xvalues,...plot_boxplot_tiltedlabels
    repmat(Cmap_patch(Conditions,:),numvariables,1),repmat(CondColors(Conditions,:),numvariables,1),...
    'k',.4,FtSz,FntName,'.');

font_style([],[],{'Mean distance from';'center of spot';'during visit (mm)'},'normal',FntName,FtSz)%params.Subs_Names{lsubs}

stats_boxplot_tiltedlabels(MeandistVFlies(:,Conditions),cell(1,1),Conditions,xvalues,...
