function h=plot_tracks_singleFig5(FlyDB,X_Y,lfly,Spots,params,px,Color,range,FontSz,plotarena,LW)
...
% % %     % [c1]=circle_([0,0],33,100,'-');
    set(c1,'Color',[0.7 0.7 0.7],'LineWidth',1)
...
if plotarena==1
    ...
    for nspot=YSpots
        [c,xc,yc]=circle_([FlyDB(lfly).WellPos(nspot,1)*params.px2mm,...
            ...
            2.5,100,'-b');%1.6
%         set(c,'Color','k','LineWidth',0.8)
%         set(c,'Color',CmapSubs(1,:),'LineWidth',1)
%         set(c,'Color',CmapSubs_Patch(1,:),'LineWidth',2)
        patch(xc,yc,CmapSubs_Patch(1,:),'EdgeColor',[.5 .5 .5])%CmapSubs_Patch(1,:))
...
%         set(c,'Color',CmapSubs_Patch(2,:),'LineWidth',2)
        patch(xc,yc,CmapSubs_Patch(2,:),'EdgeColor',[.5 .5 .5])%CmapSubs_Patch(2,:))
    end
end
...
%         set(c,'Color',CmapSubs2(Geometry(nspot),:),'LineWidth',2)
        c=circle_([FlyDB(lfly).WellPos(nspot,1)*params.px2mm,...
            FlyDB(lfly).WellPos(nspot,2)*params.px2mm],...
           2.5,100,':');% 1.5
        set(c,'Color',CmapSubs_Patch(Geometry(nspot),:),'LineWidth',.8)%[0.7 0.7 0.7]
%                 
