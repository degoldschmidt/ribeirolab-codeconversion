...
frames2test=Info.YFeedingEvents(1,1:2);
lfly=Info.(Vartoplot)(1,4);
...
RecordVideo=1;%1;%Set to 1 to save video
fps=25;%20;%40;%15
fixedframe=0;

if RecordVideo==1
    saving_dir=[DataSaving_dir_temp Exp_num '\Plots\Figures\'];
...
for n=1:19
    Dist2Spots_temp=sqrt(sum(((Heads_Sm{lfly}(Realrange,:)-...
        repmat(FlyDB(lfly).WellPos(n,:),...
        length(Realrange),1)).^2),2));
