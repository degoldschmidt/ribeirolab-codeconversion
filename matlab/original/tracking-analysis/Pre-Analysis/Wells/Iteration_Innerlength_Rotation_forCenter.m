...
[TH_T1,R_T1]=cart2pol(Template{larena}(sort(MinIdxs),1),Template{larena}(sort(MinIdxs),2));%Polar coordinates of template
...
for lrot=ranges_rotation
    ...
    Var_Rot(lrotcounter)=var(TH_T-TH_D);
    if plot_innerl_rotation==1
        %%% Plot wells
        subplot('Position',[0.07,0.1,0.4,0.8])
        axis off
...
WellPositions_All(Geometry_ALL(lfile,:)==2,2*larena-1:2*larena)=...
    wellpos_temp2(Geometry_ALL(lfile,:)==2,:)-repmat(Center_temp(larena,:),...
    sum(Geometry_ALL(lfile,:)==2),1);
%%% Save detected for yeast
WellPositions_All(Geometry_ALL(lfile,:)==1,2*larena-1:2*larena)=...
    temp_yeast-repmat(Center_temp(larena,:),...
    sum(Geometry_ALL(lfile,:)==2),1);
%%% Save template for yeast
WellPositions_All2(Geometry_ALL(lfile,:)==1,2*larena-1:2*larena)=...
    wellpos_temp2(Geometry_ALL(lfile,:)==1,:)-repmat(Center_temp(larena,:),...
    sum(Geometry_ALL(lfile,:)==1),1);
