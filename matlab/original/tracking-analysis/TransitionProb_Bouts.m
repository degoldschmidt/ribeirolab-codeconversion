function [TrEvents2B,Etho_Tr2B,Etho_Tr_Colors2, Etho_Tr2_2B,Etho_Tr2_2Colors,DurInBouts]=TransitionProb_Bouts(Heads_Sm,Walking_vec,InSpot,FlyDB,params)
...
Etho_Tr_Colors2=[241 195 27;...%1-Yeast Visit
    0 0 0;...%2- Sucrose Visit
    230 159 0;... % 3 - Same Yeast (Orange)
    255 255 255;... %4 - Same sucrose (white)
    86 80 233;... %5- Close yeast (light blue)
...
TrEvents2B=nan(numtypes,3*numtypes,size(DurInBouts,1));%rows:from substrates,cols:to same, to adjsubstrates,to farsubstrates
Etho_Tr2B=(4*numtypes+1)*ones(params.numflies,params.MinimalDuration);
Etho_Tr2_2B=(4*numtypes+2)*ones(params.numflies,params.MinimalDuration);

for lfly=1:size(DurInBouts,1)
    display(lfly)
    ...
    Tr_close_far=zeros(size(TrEvents2B,1),size(TrEvents2B,2));
    
    if size(DurInBouts{lfly},1)>=2
        for lvisit=1:size(DurInBouts{lfly},1)-1
            Spotfrom=DurInBouts{lfly}(lvisit,4);
            Spotto=DurInBouts{lfly}(lvisit+1,4);
            ...
            Etho_Tr2B(lfly,DurInBouts{lfly}(lvisit,2):DurInBouts{lfly}(lvisit,3))=find(Type_Numbers==Geometry(Spotfrom));%Geometry(Spotfrom)

            %% Transition to close or distant spot?
            Diff2fSpot=Heads_Sm{lfly}(DurInBouts{lfly}(lvisit,3)+1:DurInBouts{lfly}(lvisit+1,2)-1,:)-...
                repmat(FlyDB(lfly).WellPos(Spotfrom,:),...
                DurInBouts{lfly}(lvisit+1,2)-DurInBouts{lfly}(lvisit,3)-1,1);
            Dist2fSpots=sqrt(sum(((Diff2fSpot).^2),2)).*params.px2mm;
    %         distcovered=nansum(Steplength_Sm_c{lfly}(DurInV{lfly}(lvisit,3)+1:DurInV{lfly}(lvisit+1,2)-1))*params.px2mm/10;%cm
            ...
            Etho_Tr2B(lfly,DurInBouts{lfly}(lvisit,3)+1:DurInBouts{lfly}(lvisit+1,2)-1)=IBItype;
            Etho_Tr2_2B(lfly,DurInBouts{lfly}(lvisit+1,2):DurInBouts{lfly}(lvisit+1,3))=IBItype;
            if lvisit==1
                Etho_Tr2_2B(lfly,DurInBouts{lfly}(lvisit,2):DurInBouts{lfly}(lvisit,3))=4*numtypes+1;
            end
        end
    elseif size(DurInBouts{lfly},1)==1
        Etho_Tr2_2B(lfly,DurInBouts{lfly}(1,2):DurInBouts{lfly}(1,3))=4*numtypes+1;
    end
    TrEvents2B(:,:,lfly)=Tr_close_far;
...
Etho_Tr2_2B(Etho_Tr2_2B==l)=l-numtypes;
% 1 - Same Y
% 2 - Same S
% 3 - Close Y
% 4 - Close S
