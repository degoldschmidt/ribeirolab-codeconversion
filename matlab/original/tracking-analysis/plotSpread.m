...
%       distributionFaceColors : color identifier (string, cell array of
%           strings), or colormap, with a single color, or one color per
%           distribution (or per entry in distributionIdx). Colors the
%           distributions. Default: 'b'
...
%           Default: '.'
%       distributionMarkersSize :number
%       distributionMarkers : string, or cell array of strings, with either
%           a single marker or one marker per distribution (or per entry in
%           distributionIdx). See linespec for admissible markers.
%           Default: '.'
...
def.distributionFaceColors = 'b';
def.distributionMarkers = '.';
def.distributionMarkersSize = 6;
def.xMode = 'manual';
def.xyOri = 'normal';
def.categoryIdx = [];
def.categoryColors = [];
...
if ~isempty(varargin) && ~ischar(varargin{1}) && ~isstruct(varargin{1})
    ...
    opt.distributionFaceColors = def.distributionFaceColors;
    opt.distributionMarkers = def.distributionMarkers;
    opt.distributionMarkersSize = def.distributionMarkersSize;
    opt.xMode = def.xMode;
    opt.xyOri = def.xyOri;
    opt.categoryIdx = [];
...
if ischar(opt.distributionFaceColors)
    opt.distributionFaceColors = {opt.distributionFaceColors};
end
...
if iscell(opt.distributionFaceColors)
    if length(opt.distributionFaceColors) == 1
        % expand
        opt.distributionFaceColors = repmat(opt.distributionFaceColors,nData,1);
    elseif length(opt.distributionFaceColors) ~= nData
        error('please submit one color per distribution (%i dist, %i FaceColors)',nData,length(opt.distributionFaceColors));
    end
    
else
    if size(opt.distributionFaceColors,2) ~= 3
        error('please specify colormap with three columns')
    end
    if size(opt.distributionFaceColors,1) == 1
        opt.distributionFaceColors = repmat(opt.distributionFaceColors,nData,1);
    elseif size(opt.distributionFaceColors,1) ~= nData
        error('please submit one color per distribution (%i dist, %i FaceColors)',nData,size(opt.distributionFaceColors,1));
    end
    
    % create a cell array
    opt.distributionFaceColors = mat2cell(opt.distributionFaceColors,ones(nData,1),3);
end
...
plotFaceColors = repmat(opt.distributionFaceColors(:),1,nCategories);
plotMarkers= repmat(opt.distributionMarkers(:),1,nCategories);
plotMarkersSize=opt.distributionMarkersSize;
            ...
            switch opt.xyOri
                case 'normal'
                    ph(iData,iCategory) = plot(ah,data(currentIdx,1),...
                        ...
                        'markersize',plotMarkersSize,...
                        'color',plotColors{iData,iCategory},...
                        'markerfacecolor',plotFaceColors{iData,iCategory},...
                        'lineStyle','none',...
