...
RemoveGlobalNoneaters=1;% 0- do not do anything, 1- remove
NonEaterThreshold=2;
...
if str2double(AnswerT)==0
    ...
    %%% All CS
%     Events.ConditionLabel{1}= 'M, Yaa';
%     Events.ConditionLabel{2}= 'V, Yaa';
%     Events.ConditionLabel{3}= 'M, Hunt';
%     Events.ConditionLabel{4}= 'V, Hunt';
%     Events.ConditionLabel{5}= 'M, S';
%     Events.ConditionLabel{6}= 'V, S';
%
    %%% All CS, but Virgin Hunt
%     Events.ConditionLabel{1}= 'M, Yaa';
%     Events.ConditionLabel{2}= 'V, Yaa';
%     Events.ConditionLabel{3}= 'M, Hunt';
%     Events.ConditionLabel{4}= 'M, S';
%     Events.ConditionLabel{5}= 'V, S';
%     
    %%% Only MatedCS for Microstructure
%     Events.ConditionLabel{1}= 'M, Yaa';
%     Events.ConditionLabel{2}= 'M, Hunt';
%     Events.ConditionLabel{3}= 'M, S';
%     
    %%% All Tbh experiment
%     Events.ConditionLabel{1}= 'CSM, Hunt';
%     Events.ConditionLabel{2}= 'CSV, Hunt';
%     Events.ConditionLabel{3}= 'CSM, S';
%     Events.ConditionLabel{4}= 'CSV, S';
%     Events.ConditionLabel{5}= 'TbhM, Hunt';
%     Events.ConditionLabel{6}= 'TbhV, Hunt';
%     Events.ConditionLabel{7}= 'TbhM, S';
%     Events.ConditionLabel{8}= 'TbhV, S';
%     
    %%% for the Tbh Virgins experiment
%     Events.ConditionLabel{1}= 'CSV, Hunt';
%     Events.ConditionLabel{2}= 'CSV, S';
%     Events.ConditionLabel{3}= 'TbhV, Hunt';
%     Events.ConditionLabel{4}= 'TbhV, S';
%
    %%% Tbh postmating in deprived females
    Events.ConditionLabel{1}= 'CSM, S';
    Events.ConditionLabel{2}= 'CSV, S';
    Events.ConditionLabel{3}= 'TbhM, S';
    Events.ConditionLabel{4}= 'TbhV, S';
    
    
    
    %% These are the labels for substrates (what goes on channel 1 and 2)
    Events.SubstrateLabel{1}='Sucrose 18%';
    Events.SubstrateLabel{2}='Yeast 18%';
...
%% Detect errors with Condition Labels 
display('Double checking condition labels...')
CapFilenames_temp=fileList(logical(z));
ncondmax=99;

for FileNameCounter=1:size(CapFilenames_temp,1)
    CapFilename_temp=CapFilenames_temp{FileNameCounter};
    CapFilename_temp=CapFilename_temp(numel(currentDir)+1:end)
    C_temp=nan(ncondmax,2);
    for n = 1:ncondmax 
        S=['C' num2str(n, '%02i')];
        C_temp(n,1)=str2double([CapFilename_temp(strfind(CapFilename_temp,S)+4) CapFilename_temp(strfind(CapFilename_temp,S)+5)]);
        C_temp(n,2)=str2double([CapFilename_temp(strfind(CapFilename_temp,S)+7) CapFilename_temp(strfind(CapFilename_temp,S)+8)]);
    end
    C_temp=C_temp(~isnan(C_temp(:,1)),:);
    if length(Events.ConditionLabel)~=size(C_temp,1)
        error('ErrorTAG:TagName', ['Nº of conditions in the ',...
            'capacitance filename above: ',num2str(size(C_temp,1)),...
            '. Nº of conditions in ''Events.ConditionLabel'': ',...
            num2str(length(Events.ConditionLabel)) ' --> They don''t match! ',...
        'Double check capacitance filenames and that Condition labels are ok.',...
        ' Check that in ''Events.ConditionLabel{n}'', n goes from 1 to the ',...
        'correct Nº of conditions'])
    end
